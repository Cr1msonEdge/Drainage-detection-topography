...
def create_patch_dataset_from_tif(image_path: str, mask_path: str, name: str, patch_size: int = 256, thresh: float = 0.3):
    ...
    with rasterio.open(image_path) as img_src:
        ...
        for top in tqdm(range(0, img_height - patch_size + 1, patch_size), desc="Checking image patches"):
            for left in range(0, img_width - patch_size + 1, patch_size):
    ...
    with rasterio.open(mask_path) as mask_src:
        for (left, top) in tqdm(valid_indices, desc="Extracting mask patches"):
            window = Window(left, top, patch_size, patch_size)
            mask = mask_src.read(1, window=window)  # Одноканальная маска
